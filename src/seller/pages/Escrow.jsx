      ...
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col max-h-[90vh] animate-slide-up">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-black text-neutral-900 text-sm leading-none">{escrow.id}</h3>
              <p className="text-[10px] text-neutral-400 mt-0.5">{escrow.orderId} · {escrow.customer}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          ...
          <div className="bg-linear-to-br from-purple-50 to-purple-100/50 border border-purple-200 rounded-2xl p-4 text-center space-y-1.5">
            <p className="text-3xl font-black text-neutral-900">{fmt(escrow.amount)}</p>
          ...
          <div className="bg-white rounded-2xl border border-neutral-100 divide-y divide-neutral-50 overflow-hidden">
            {[
              ...
              <div key={k} className="flex justify-between items-center px-4 py-2.5 text-sm">
                ...
                <span className="font-bold text-neutral-800 text-right">{v}</span>
              </div>
            ))}
          </div>
          ...
          {escrow.note && (
            <div className="flex gap-2 bg-blue-50 border border-blue-100 rounded-xl p-3">
              ...
              <p className="text-xs text-neutral-500 leading-relaxed">{escrow.note}</p>
            </div>
          )}

          {/* Release timeline */}
          <div className="space-y-1.5">
            <p className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Release Timeline</p>
              ...
              {TIMELINE.map((step, i) => (
                <div key={i} className="flex items-center gap-3 py-2">
                  <div className="flex flex-col items-center shrink-0">
                    ...
                    {i < TIMELINE.length - 1 && (
                      <div className={`w-0.5 h-5 mt-0.5 rounded ${step.done ? "bg-green-300" : "bg-neutral-200"}`} />
                    )}
                  </div>
                  <p className={`text-sm font-medium ${step.done ? "text-neutral-800" : "text-neutral-400"}`}>
                    {step.label}
                  </p>
                </div>
              ))}
...
export default function EscrowPage() {
  ...
  const fetchEscrowData = async () => {
    ...
    try {
      const [summaryRes, listRes] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL}/api/seller/escrow/summary`, { credentials: "include" }),
        fetch(`${import.meta.env.VITE_API_URL}/api/seller/escrow`, { credentials: "include" }),
      ]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-black text-neutral-900">Escrow</h1>
        <p className="text-xs text-neutral-400">Track funds held during active orders</p>
      </div>
      ...
      <div className="flex gap-3 bg-blue-50 border border-blue-200 rounded-2xl p-4">
        ...
        <div>
          <p className="text-sm font-bold text-neutral-800">7-Day Auto-Release</p>
          <p className="text-xs text-neutral-500 mt-0.5 leading-relaxed">
      ...
      <div className="space-y-2 min-h-[300px]">
        ...
        filtered.map((escrow) => (
          <div
            key={escrow._id}
            className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-4 hover:shadow-md hover:border-neutral-200 transition-all"
          >
              <div
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <p className="text-sm font-bold text-neutral-900 truncate">{escrow.product}</p>
                    <StatusBadge status={escrow.status} />
                  </div>
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-black text-neutral-900">{fmt(escrow.amount)}</p>
                  <p className="text-[10px] text-neutral-400 capitalize mt-0.5">{escrow.deliveryStatus}</p>
                </div>
              </div>
